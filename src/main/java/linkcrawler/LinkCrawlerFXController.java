...
public class LinkCrawlerFXController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		 ...
		 else if(location.toString().contains("authDialog")) //$NON-NLS-1$
		 {
			 httpusernameTextField.setText(httpUsername);
			 httppasswordTextField.setText(httpPassword);
		 }
		 else if(location.toString().contains("about")) //$NON-NLS-1$
		 {
			// Image imageFB = new Image(this.getClass().getResource("fb.png").toExternalForm());
	...
	protected void startCrawling(ActionEvent event) {
		...
		engineStatus.setText(Messages.getString("LinkCrawlerFXController.12")); //$NON-NLS-1$
	    ...
	    {
	    	LinkCrawlerFXDialogs.showErrorDialog(Messages.getString("LinkCrawlerFXController.13"), Messages.getString("LinkCrawlerFXController.14"));	         //$NON-NLS-1$ //$NON-NLS-2$
	        enableCriticalElements();
	        engineStatus.setText(Messages.getString("LinkCrawlerFXController.15")); //$NON-NLS-1$
	    ...
	    {
	        case NOTVALID:
	            message = Messages.getString("LinkCrawlerFXController.17"); //$NON-NLS-1$
	            break;
	        case MALFORMED:
	    ...
	    {
	    	LinkCrawlerFXDialogs.showErrorDialog(Messages.getString("LinkCrawlerFXController.22"), message);	         //$NON-NLS-1$
	        enableCriticalElements();
	        engineStatus.setText(Messages.getString("LinkCrawlerFXController.23")); //$NON-NLS-1$
	        progressAnimation.visibleProperty().set(false);
	        return;
	    }		
	    ...
	    {       
	        LinkCrawlerFXDialogs.showErrorDialog(Messages.getString("LinkCrawlerFXController.28"), Messages.getString("LinkCrawlerFXController.29")); //$NON-NLS-1$ //$NON-NLS-2$
	        engineStatus.setText(Messages.getString("LinkCrawlerFXController.30")); //$NON-NLS-1$
	        progressAnimation.visibleProperty().set(false);
	        return;
	    }
	    statusMessageLabel.setText(Messages.getString("LinkCrawlerFXController.31")); //$NON-NLS-1$
	    ...
	    mainlog.addOutputLine(Messages.getString("LinkCrawlerFXController.33"), Messages.getString("LinkCrawlerFXController.34")); //$NON-NLS-1$ //$NON-NLS-2$
		
	    Configuration config = new Configuration();
		String depthLevel = depthLevelSpinner.getText().trim();
		if (depthLevel.equals(Messages.getString("LinkCrawlerFXController.35").trim())) //$NON-NLS-1$
		{
		...
		config.setName("SiteName"); //$NON-NLS-1$
		config.setExclusionListArray(exclusionListArray);
		config.setDomain(domainToCheck);
		config.setCheckSubdomains(!checkSubdomainsCheckBox.isSelected());//勾选则全面检查子站
		config.setImageCheck(enableImageCheckingCheckbox.isSelected());
		config.setBrowserName(browserSelection.getValue());
		
		
		config.setTelephone("15850675602"); //$NON-NLS-1$
		config.setEmail("15850675602@139.com"); //$NON-NLS-1$
		config.setEmailHost("smtp.163.com"); //$NON-NLS-1$
		config.setSendEmail("dalei_main2000@163.com"); //$NON-NLS-1$
		config.setEmailPassword("fanghong2006"); //$NON-NLS-1$
                    ...
                    Platform.runLater(new Runnable() {
                        ...
                        public void run() {                        	
                        	...
                        	else
                        	{
                        		 ...
                        		 statusMessageLabel.setText(Messages.getString("LinkCrawlerFXController.46")); //$NON-NLS-1$
                                 engineStatus.setText(Messages.getString("LinkCrawlerFXController.47")); //$NON-NLS-1$
                                 progressAnimation.visibleProperty().set(false);
                                 enableCriticalElements();
                                 runSignal = 0;
                        	}
        ...
        crawlUpdateUIThread.setName("UI Updater"); //$NON-NLS-1$
	...
	protected void stopCrawling(ActionEvent event) {
		if(job.isAlive())
		{
			engineStatus.setText(Messages.getString("LinkCrawlerFXController.49")); //$NON-NLS-1$
			job.stopCrawling();
	...
	protected void setCredentialsWindow(ActionEvent event) {
		...
		dialogStage.setTitle(Messages.getString("LinkCrawlerFXController.51")); //$NON-NLS-1$
	...
	protected void openAboutWindows(ActionEvent event) {
		...
		aboutStage.setTitle(Messages.getString("LinkCrawlerFXController.55")); //$NON-NLS-1$
	...
	protected void openExclusionWindows(ActionEvent event) {
		...
		exclusionStage.setTitle(Messages.getString("LinkCrawlerFXController.59")); //$NON-NLS-1$
	...
	protected void clearCredentialsAction(ActionEvent event) {
		...
		if(httpUsername.trim().equals("") && httpPassword.trim().equals("")) //$NON-NLS-1$ //$NON-NLS-2$
		{
			credentialsToUse.setText(Messages.getString("LinkCrawlerFXController.0")); //$NON-NLS-1$
		}
		else
		{
			credentialsToUse.setText(httpUsername);
	...
	protected void addStringForExclusion(ActionEvent event) {
		...
		{
			LinkCrawlerFXDialogs.showErrorDialog(Messages.getString("LinkCrawlerFXController.70"), Messages.getString("LinkCrawlerFXController.71")); //$NON-NLS-1$ //$NON-NLS-2$
			return;
		}
		if(exclusionListArray.contains(stringToAdd))
		{
			LinkCrawlerFXDialogs.showErrorDialog(Messages.getString("LinkCrawlerFXController.72"), Messages.getString("LinkCrawlerFXController.73")); //$NON-NLS-1$ //$NON-NLS-2$
			return;
		}
		exclusionListArray.add(stringToAdd);
		exclusionListView.getItems().add(stringToAdd);
		exclusionListStatusLabel.setText(Messages.getString("LinkCrawlerFXController.74")); //$NON-NLS-1$
	...
	protected void clearStringForExclusion(ActionEvent event) {
		...
		exclusionListStatusLabel.setText(Messages.getString("LinkCrawlerFXController.75")); //$NON-NLS-1$
	...
	protected void goToHomePage(ActionEvent event) {
		if (Desktop.isDesktopSupported()) {
		      try {
		        Desktop.getDesktop().browse(new URL("https://github.com/fanghon").toURI()); //$NON-NLS-1$
		      } catch (Exception ex) { /* TODO: error handling */ }
		    } else { /* TODO: error handling */ }		
	}
	...
	protected void htmlReportMenuItemActionPerformed(ActionEvent event) {
        ...
        disableCriticalElements();
        statusMessageLabel.setText(Messages.getString("LinkCrawlerFXController.1")); //$NON-NLS-1$
        rc.start();
        runSignal = 1;
	...
	protected void excelReportMenuItemActionPerformed(ActionEvent event) {
        ...
        disableCriticalElements();
        statusMessageLabel.setText(Messages.getString("LinkCrawlerFXController.80")); //$NON-NLS-1$
        rc.start();
        runSignal = 1;        
        createReportUpdateThread();        
        reportUpdateUIThread.start();
                    ...
                    Platform.runLater(new Runnable() {
                        ...
                        public void run() {
                        	...
                        	else
                        	{
                        		...
                        		if(!rc.isSuccess())
                                {
                        			statusMessageLabel.setText(Messages.getString("LinkCrawlerFXController.81")); //$NON-NLS-1$
                        			LinkCrawlerFXDialogs.showErrorDialog(Messages.getString("LinkCrawlerFXController.82"), Messages.getString("LinkCrawlerFXController.83") + rc.getException());                                 //$NON-NLS-1$ //$NON-NLS-2$
                                }
                                else
                                {
                                	statusMessageLabel.setText(Messages.getString("LinkCrawlerFXController.84")); //$NON-NLS-1$
                                	LinkCrawlerFXDialogs.showInfoDialog(Messages.getString("LinkCrawlerFXController.85"), Messages.getString("LinkCrawlerFXController.86") + rc.getReportLocation()); //$NON-NLS-1$ //$NON-NLS-2$
                                }                    		
                        	}
	...
	private void clearPieChart()
	{
		ObservableList<PieChart.Data> pieChartData =
                ...
                new PieChart.Data(Messages.getString("LinkCrawlerFXController.87"), 0), //$NON-NLS-1$
                new PieChart.Data(Messages.getString("LinkCrawlerFXController.88"), 0)); //$NON-NLS-1$
